...
fun.biplot <-
    function (x, y, var.axes = TRUE, cex = rep(par("cex"), 2),
              xlabs = NULL, ylabs = NULL, expand = 1,
              xlim  = NULL, ylim  = NULL, arrow.len = 0.1,
              main = NULL, sub = NULL, xlab = NULL, ylab = NULL,
              col, choices, scale, pc.biplot, col.obs, etich, ...)
{
    ...
    if (pc.biplot)
        lam <- lam/sqrt(n)
    fun.MyBiplot(t(t(scores[, choices])/lam), t(t(x$loadings[,
                                                             choices]) * lam), col.obs = col.obs, xlabs=etich, ...)
    invisible()
}
fun.MyBiplot <-
              ...
              col.obs, col, ...)
{
    ...
    text(x, xlabs, cex = cex[1L], col = col.obs, ...)# col[1L], ...)
    par(new = TRUE)
    dev.hold()
