...
\SweaveOpts{concordance=TRUE, prefix.string=Grafici/FIG}
\usepackage{siunitx}
...
# ifelse(info.sistema[3] == "Lorenzo Ferretti",  "Lorenzo Ferretti.LAPTOP-LDRBKLR9",  info.sistema[3])
 ...
 if(info.sistema[2]=="LAPTOP-LDRBKLR9"){
   DirMain <- paste("C:/Users/", info.sistema[3], ".", info.sistema[2],"/Documents/GitHub/PaperoSuoloMPaldi", sep="")
   Sys.setenv(TEXINPUTS=getwd(), BIBINPUTS=getwd(), BSTINPUTS=getwd())
 }else{
   if(info.sistema[2]=="DESKTOP-VAAU252"){
     DirMain <- paste("C:/Users/", info.sistema[3], "/Documents/GitHub/PaperoSuoloMPaldi", sep="")
     Sys.setenv(TEXINPUTS=getwd(), BIBINPUTS=getwd(), BSTINPUTS=getwd())
   } else {
     if(info.sistema[3]=="ottorino"){
...
## source(file.path(DirFunz, "Pettinatura_df.R"))
...
<<label=printBoxplotCore, fig=TRUE, echo=FALSE, width=5.4, height=5.4 >>=
plot(1:10)
@ 
...
\bibliography{../tesi/bibliomoneDali.bib} % name your BibTeX data base
