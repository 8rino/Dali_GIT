...
## debug(fun.tabellone)
## si salvano i parametri grafici che ci comodano dentro l'oggetto my.par
my.par <- par(cex.axis =1.5,  cex.lab=1.5, cex=1.5)
@
...
\todo{controllare bibliografia e scegliere rivista: applied soil ecology e, 
potenzialmente SBB}
\todo{controllare format richiesta rivista (latex ok in entrambe), controlla 
struttura}
\todo{buttare giu max 3 conclusioni e scrivere all'indietro}
\todo{controllare di avere tutte le figure e tabelle per le conclusioni}
\todo{modificare tabelle e immagini:inglese-ita e dimensione testo}
\todo{scheletro e controlla riviste}
...
<<FigRapportiTILL, fig=FALSE, echo=FALSE, results = tex >>=
par(my.par)
...
fattore <-interaction(lis.tutto$CATEGORIA$MAN, lis.tutto$CATEGORIA$TIL, lis.tutto$CATEGORIA$SEASON)
...
labs <- levels(fattore)
text(cex=1, x=(1:12), y=-0.15, labs, xpd=TRUE, srt=90)
...
<<label=FigRapportiTILL, fig=TRUE,  echo=FALSE>>=
<<FigRapportiTILL>>
@
...
fattore <- interaction(lis.tutto$CATEGORIA$TIL, lis.tutto$CATEGORIA$SEASON)
...
ordina <- with(df.disastro, order(SEASON, MAN,TIL))
...
row.names(df.butta) <-
    with(lis.tutto$CATEGORIA,
         paste(substr(as.character(SEASON), 1,2),
...
ordina <-
    with(lis.tutto$CATEGORIA, order(SEASON, MAN,TIL, FIELD))
...
@
 ...
 \begin{figure}[h]
   \centering
<<label=StarplotCampioni, fig=TRUE, echo=FALSE, eval=FALSE>>=
<<FigStarplotCampioni>>
@
  % \caption[Starplot dei 72 campioni]
{Ogni elemento (starplot) mostra
la composizione in BAMEs di uno dei 72
campioni, ogni raggio rappresenta uno dei 36 BAMEs identificati.
Il BAMEs con concentrazione maggiore è posto uguale ad uno e gli
...
fattore <-
    interaction(lis.tutto$CATEGORIA$TIL, lis.tutto$CATEGORIA$SEASON)
...
#      c("(Intercetta)", "LAV Rip", "LAV Fzo","SEASON Est")
...
xc <-
    aggregate(Y.MOLI,
              by = list(SEASON = lis.tutto$CATEGORIA$SEASON,
                        MAN = lis.tutto$CATEGORIA$MAN,
                        TIL = lis.tutto$CATEGORIA$TIL),
              mean)

row.names(xc) <-
    with(xc, as.character(interaction(SEASON, MAN, TIL)))
...
#     c("(Intercetta)", "GESTIONE", "SEASON", "LAVORAZIONE",
...
stag <- lis.tutto$CATEGORIA$SEASON
til <- lis.tutto$CATEGORIA$TIL
man <- lis.tutto$CATEGORIA$MAN
...
fattore <-
    interaction(lis.tutto$CATEGORIA$SEASON,
                lis.tutto$CATEGORIA$TIL,
                lis.tutto$CATEGORIA$MAN)
...
lm.1 <-
    lm(ilr(Y.prova) ~  SEASON, data = lis.tutto$CATEGORIA)
anova(lm.1)
...
\begin{table}[h]
\caption[Physico-chemical characteristics of soils used in this experiment]
{[Physico-chemical characteristics of soils used in this experiment]\citep{DISPAA}. }
 \label{tab:CarattMoLTE1992}
\centering
  \begin{tabular}{llll}
\hline\noalign{\smallskip}
    Parameter & Unit & Organic & Conventional   \\
\noalign{\smallskip}\hline\noalign{\smallskip}
    clay            & \%                & 32.9 & 33.8 \\
    silt            & \%                & 46.3 & 44.6 \\
    sand            & \%                & 20.2 & 21.0 \\
%%  gravel          & \%                &  6,3 &  6,1 \\
    pH$(_{H_2 O})$   &                   &  8.3   &  8.3 \\
    CEC             & \si{\cmol\per\kg} & 17.6   & 19.4 \\
    total \ce{P2O5} & \%                &  1.63  & 1.60 \\
    total \ce{K2O}  & \si{\mg\per\kg  } & 171    & 134 \\
\noalign{\smallskip}\hline
  \end{tabular}
\end{table}
...
in the lab. \migliora[inline]{Nei riconoiscimenti ci andr\`a messo
